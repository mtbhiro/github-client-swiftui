...
struct GitHubAuthServiceTests {
    ...
    private func makeSUT(clientID: String? = "test-client-id") -> SUT {
        ...
        let keychain = KeychainStorage(service: "test.\(UUID().uuidString)", account: "token")
        ...
        return SUT(service: service, oauth: oauth, api: api)
    }
    ...
    @Test func requestDeviceCode_success_returnsDomain() async throws {
        ...
        let code = try await sut.service.requestDeviceCode()
    ...
    @Test func requestDeviceCode_emptyClientID_throwsConfigError() async {
        ...
        await #expect(throws: GitHubAuthConfigError.missingClientID) {
            _ = try await sut.service.requestDeviceCode()
        }
    }
    ...
    @Test func pollAccessToken_success_returnsToken() async throws {
        ...
        let outcome = try await sut.service.pollAccessToken(deviceCode: "dc")
    ...
    @Test func pollAccessToken_includesDeviceCode_andGrantType() async throws {
        ...
        _ = try await sut.service.pollAccessToken(deviceCode: "dev-code-123")
    ...
    @Test func fetchAuthenticatedUser_success_returnsUser() async throws {
        ...
        let user = try await sut.service.fetchAuthenticatedUser(token: "gho_xxx")
    ...
    @Test func saveAndLoadToken_roundTrips() throws {
        let sut = makeSUT()
        try sut.service.saveToken("gho_yyy")
        #expect(sut.service.loadToken() == "gho_yyy")
    }

    @Test func clearToken_removesValue() throws {
        let sut = makeSUT()
        try sut.service.saveToken("gho_zzz")
        try sut.service.clearToken()
        #expect(sut.service.loadToken() == nil)
    }
}
