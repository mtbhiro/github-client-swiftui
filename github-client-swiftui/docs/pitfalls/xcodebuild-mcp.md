...
## `test_sim` をタスク単位で何度も呼んでシミュレータがゾンビ化する

### 症状

開発スキル（`/developer` 等）で 1 タスク = 1 つのテストファイル単位に Red → Green → Refactor を回していると、各タスクの末尾に `mcp__XcodeBuildMCP__test_sim` を `-only-testing:...` で呼ぶ運用になりがち。タスクが 7 〜 10 個あると **その回数だけシミュレータがブートされ**、ユーザー環境に iPhone 17 シミュレータが多数残り、Mac のリソース（メモリ・GPU・Simulator サービス）を食い潰す。

`xcrun simctl shutdown all` を後追いで叩けば回収できるが、それ自体が無駄な操作。

### 原因

XcodeBuildMCP の `test_sim` は呼び出しごとに対象 simulator を boot する。すでに boot 済みの simulator があっても、別 simulator UDID なら別インスタンスが立つ。同一 UDID でも、xcodebuild の test 実行が走るたびに `Simulator.app` の window がフォアグラウンドに来てフォーカスを奪う。1 回が低コストでも、テスト戦略として「タスクごとに `test_sim`」を選ぶと累積で大きい。

### 回避策

**`-only-testing:...` を 1 タスクあたり 1 回だけ呼ぶ運用をやめ、新規追加したテストをまとめて 1 回の `test_sim` で流す**。具体的には:

1. **新しいテストを書くたびに即実走しない**。Red → Green → Refactor を「テストを書いて → 実装して → ローカルでビルドを軽く通す」ところまで進め、テスト実走は「ある程度テストが溜まってから」もしくは「設計判断に直結する箇所」のみに絞る。
2. **複数タスクで作ったテストを 1 回でまとめて流す**。`extraArgs: ["-only-testing:github-client-swiftuiTests/A", "-only-testing:github-client-swiftuiTests/B", ...]` を 1 回の `test_sim` 呼び出しに集約する。
3. **ビルドが通ることだけ確認したいなら `build_sim` を `extraArgs: ["build-for-testing"]` で叩く**。テストランナーは起動しないので simulator が boot されない。型エラー検知だけならこれで十分。
4. **フェーズ 5 の最終確認では `-only-testing` を外して全体を 1 回で流す**。これで「累積 7 回」が「1 〜 2 回」に圧縮できる。

判断軸: 「**追加したテストを今すぐ通したい根拠**」があるときだけ `test_sim` を呼ぶ。Red の確認・並列レース調査・テスト同士の干渉切り分けなど、実走しないと分からないケースに限定する。型エラーや warning は build_sim で取れる。

### 一次ソース

- セッション内の実観測。`xcodebuild test` 自体は 1 回呼びで N test を含められるため、複数呼び出しに分ける技術的必要性は無い
- [`xcodebuild` man page](https://keith.github.io/xcode-man-pages/xcodebuild.1.html) — `-only-testing:TEST-IDENTIFIER` は **繰り返し** 指定可能で、複数を 1 回の invocation に含めて良い
